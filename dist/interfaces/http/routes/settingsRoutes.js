...
router.delete('/smtp', auth_1.authenticate, (0, auth_1.authorize)('ADMIN'), async (req, res) => {
    try {
        await settingsRepository_1.settingsRepository.deleteByCategory('smtp');
...
router.get('/', auth_1.authenticate, async (req, res) => {
    try {
        const settings = await settingsRepository_1.settingsRepository.findAll();
...
router.get('/category/:category', auth_1.authenticate, async (req, res) => {
    try {
        const settings = await settingsRepository_1.settingsRepository.findByCategory(req.params.category);
        const sanitized = settings.map(s => ({
            key: s.key,
            value: s.isSecret ? (s.value ? '••••••••' : '') : s.value,
            category: s.category,
...
router.put('/', auth_1.authenticate, (0, auth_1.authorize)('ADMIN'), async (req, res) => {
    try {
        ...
        const results = await Promise.all(settings.map(async (s) => {
            const setting = await settingsRepository_1.settingsRepository.upsert({
                key: s.key,
                value: s.value,
                category: s.category,
                isSecret: s.isSecret
...
// Initialize settings
router.post('/initialize', auth_1.authenticate, (0, auth_1.authorize)('ADMIN'), async (req, res) => {
    try {
...
router.get('/email', auth_1.authenticate, async (req, res) => {
    try {
        const settings = await settingsRepository_1.settingsRepository.findByCategory('smtp');
        const config = {};
...
// Save email settings
router.put('/email', auth_1.authenticate, (0, auth_1.authorize)('ADMIN'), async (req, res) => {
    try {
        ...
        await settingsRepository_1.settingsRepository.upsert({ key: 'smtp_host', value: cleanHost, category: 'smtp' });
        await settingsRepository_1.settingsRepository.upsert({ key: 'smtp_port', value: String(port || 587), category: 'smtp' });
        await settingsRepository_1.settingsRepository.upsert({ key: 'smtp_secure', value: secure ? 'true' : 'false', category: 'smtp' });
        await settingsRepository_1.settingsRepository.upsert({ key: 'smtp_user', value: cleanUser, category: 'smtp' });
        if (cleanPassword) {
            await settingsRepository_1.settingsRepository.upsert({ key: 'smtp_pass', value: cleanPassword, category: 'smtp', isSecret: true });
        }
        if (cleanFrom) {
            await settingsRepository_1.settingsRepository.upsert({ key: 'smtp_from', value: cleanFrom, category: 'smtp' });
        }
...
// Test email
router.post('/test-email', auth_1.authenticate, (0, auth_1.authorize)('ADMIN'), async (req, res) => {
    try {
        const { to } = req.body;
        if (!to) {
        ...
        const result = await emailService_1.emailService.send({
            to,
            subject: 'Prueba de configuración SMTP - PROCURA',
            html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h2 style="color: #4f46e5;">Prueba de configuración SMTP</h2>
          <p>Este es un email de prueba enviado desde PROCURA.</p>
...
router.get('/company', auth_1.authenticate, async (req, res) => {
    try {
        const settings = await settingsRepository_1.settingsRepository.findByCategory('company');
        const config = {};
        for (const s of settings) {
...
router.put('/company', auth_1.authenticate, (0, auth_1.authorize)('ADMIN'), async (req, res) => {
    try {
        const { name, rut, address, phone, email: compEmail } = req.body;
        if (name)
            await settingsRepository_1.settingsRepository.upsert({ key: 'company_name', value: name, category: 'company' });
        if (rut)
            await settingsRepository_1.settingsRepository.upsert({ key: 'company_rut', value: rut, category: 'company' });
        if (address)
            await settingsRepository_1.settingsRepository.upsert({ key: 'company_address', value: address, category: 'company' });
        if (phone)
            await settingsRepository_1.settingsRepository.upsert({ key: 'company_phone', value: phone, category: 'company' });
        if (compEmail)
            await settingsRepository_1.settingsRepository.upsert({ key: 'company_email', value: compEmail, category: 'company' });
...
router.put('/:key', auth_1.authenticate, (0, auth_1.authorize)('ADMIN'), async (req, res) => {
    try {
