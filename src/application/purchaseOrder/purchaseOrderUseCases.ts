...
export class PurchaseOrderUseCases {
  ...
  async createPurchaseOrder(data: CreatePurchaseOrderDTO, createdById: string): Promise<PurchaseOrder> {
    const order = await purchaseOrderRepository.create(data, createdById);
    
    // Notificación al proveedor
    const supplier = await supplierRepository.findById(data.supplierId);
    if (supplier && supplier.email) {
      emailService.notifyPurchaseOrderCreated(order, supplier);
    }
  ...
  async updateStatus(id: string, status: string, changedById: string, notes?: string): Promise<PurchaseOrder> {
    ...
    const updatedOrder = await purchaseOrderRepository.updateStatus(id, status, changedById, notes);
    
    // Notificaciones según el estado
    if (status === 'CONFIRMED') {
      emailService.notifyPurchaseOrderConfirmed(updatedOrder);
    } else if (status === 'RECEIVED') {
      emailService.notifyPurchaseOrderReceived(updatedOrder);
    }
