...
class SettingsRepository {
    async findAll() {
        return client_1.default.settings.findMany({
            orderBy: [{ category: 'asc' }, { key: 'asc' }],
        });
    }
    async findByKey(key) {
        return client_1.default.settings.findUnique({
            where: { key },
        });
    }
    ...
    async upsert(key, value, category = 'general', description) {
        return client_1.default.settings.upsert({
            where: { key },
    ...
    async delete(key) {
        await client_1.default.settings.delete({
            where: { key },
        });
    }
