...
export class PurchaseOrderController {
  async getAll(req: Request, res: Response) {
    try {
      ...
      let orders;
      if (supplierId) {
        orders = await purchaseOrderUseCases.getPurchaseOrdersBySupplier(supplierId as string);
      } else if (status) {
        orders = await purchaseOrderUseCases.getPurchaseOrdersByStatus(status as string);
      } else {
        orders = await purchaseOrderUseCases.getAllPurchaseOrders();
      }
...
export class PurchaseReceiptController {
  async getAll(req: Request, res: Response) {
    try {
      ...
      let receipts = await purchaseReceiptUseCases.getAllReceipts();

      if (purchaseOrderId) {
        receipts = receipts.filter((r: any) => r.purchaseOrderId === purchaseOrderId);
      }
